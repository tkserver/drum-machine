  ...
  return (
    <div className="flex-1 p-6 overflow-auto">
      <div className="max-w-3xl mx-auto">
        <h2 className="font-pixel text-sm text-muted-foreground uppercase tracking-wider mb-4">
          Drum Pads
        </h2>
          ...
          <div className="grid grid-cols-4 gap-3">
            {KEY_LABELS.map((key, i) => (
              <div key={key} className="flex items-center gap-2">
                <span className="w-8 h-8 flex items-center justify-center bg-muted border-2 border-border font-pixel text-sm text-foreground">
                  {key}
                </span>
                <span className="font-pixel-body text-lg text-muted-foreground truncate">
                  {sounds[i]?.name || '-'}
                </span>
              </div>
            ))}
